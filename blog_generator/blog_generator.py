...
# 导入配置
from blog_generator.config.config import blog_config

# 导入GitHub图床上传器 - 使用绝对路径
...
class BlogGenerator:
    ...
    def __init__(self):
        """初始化博客生成器"""
        # 从配置中加载博客设置
        self.enabled = blog_config.get('blog', 'enabled', True)
        self.template_path = blog_config.get('blog', 'template_file', 'config/templates/blog_template.md')
...
# 导出博客生成器实例，方便其他模块导入
__all__ = ['BlogGenerator', 'blog_generator']