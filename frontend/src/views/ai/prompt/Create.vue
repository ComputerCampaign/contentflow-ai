...
const handleSave = async () => {
  ...
  try {
    ...
    // 检查标识是否已存在
    if (aiStore.promptsConfig.prompts[formData.key]) {
      ElMessage.error('提示词标识已存在')
      return
    }
    
    // 添加到配置中
    aiStore.promptsConfig.prompts[formData.key] = {
    ...
    if (formData.is_default) {
      aiStore.promptsConfig.default_prompt = formData.key
    }
